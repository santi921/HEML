...
def move_charges_into_folder(charges_root, compressed_frame_folder):
    ...
    folders = [
        f
        for f in os.listdir(compressed_frame_folder)
        if os.path.isdir(os.path.join(compressed_frame_folder, f))
    ]
    for i in folders:
        frame = i.split("_")[0]
        protein = i.split("_")[1]
        ...
        if os.path.exists(
            os.path.join(charges_root, frame + "_" + protein + "_movie.pqr")
        ):
            ...
            os.system(
                ...
                + os.path.join(charges_root, frame + "_" + protein + "_movie.pqr")
                + " "
                + os.path.join(compressed_frame_folder, i)
            )
...
def extract_heme_and_ligand_from_pdb(
    root, file, add_oh=False, add_o=False, freeze=False
):
    """
    ...
    if not ligand_none:
        if ligand_dict["best_crit_dist"] > 4.0:
            ...
            print(
                f"ERROR: No cysteine/tyrosine/histine ligand found for {file_folder}.\n"
            )
            fail += 1
            # continue
        ...
        for line in f:
            ...
            if len(line_split) > 3:
                ...
                if "HETATM" in line_split:
                    ...
                    ligand_id_cond = (
                        line[22:26].strip() == fe_dict["id"].split(":")[1].strip()
                    )
                    distance = np.linalg.norm(
                        get_element_and_xyz(line, freeze=False)["xyz"] - fe_dict["xyz"]
                    )
                ...
                if sg_cond or oh_cond or nend_cond:
                    ligand_chain_cond = (
                        line[21] == ligand_dict["best_crit"].split(":")[0]
                    )
                    ligand_id_cond = (
                        line[22:26].strip()
                        == ligand_dict["best_crit"].split(":")[1].strip()
                    )
    ...
    if freeze:
        ...
        carbon_list = [
            np.linalg.norm(x["xyz"] - out_list[0]["xyz"]) for x in carbon_list
        ]
        # print(dot_list)
...
def xtb_sanitize_and_save(
    folder, name, dict_xyz, add_oh=False, add_o=False, traj_name=False
):
    """
    ...
    atoms = Atoms(
        numbers=elements, positions=positions, constraint=FixAtoms(mask=fixed)
    )
...
def write_dict_to_xyz(folder, name, dict_xyz, add_oh=False, add_o=False):
    ...
    with open(xyz_file_name, "w") as f:
        ...
        for i in dict_xyz:
            f.write(
                "{} {:.3f}  {:.3f}  {:.3f}\n".format(
                    i["element"], i["xyz"][0], i["xyz"][1], i["xyz"][2]
                )
            )
