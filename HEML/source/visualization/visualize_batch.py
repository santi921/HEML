    ...
    for file in os.listdir(os.path.join(os.path.dirname(__file__), "cpet")):
        if file.split(".")[-1] == "txt" and "field" in file:
            ...
            json_name = os.path.join(
                ...
                "cpet",
                "plot_options_{run_index}_{run_ind}_{protein}_{run}_{frame}.json".format(
                    run_index=run_index,
                    run_ind=run_ind,
                    protein=protein,
                    run=run,
            ...
            field_file = os.path.join(
                os.path.dirname(__file__),
                "cpet",
                "efield_cox_{run_index}_{run_ind}_{protein}-run{run}-{frame}.dat".format(
                    run_index=run_index,
                    run_ind=run_ind,
                    protein=protein,
                    run=run,
            ...
            pdb_file = os.path.join(
                os.path.dirname(__file__),
                "frames",
                "{run_index}_{run_ind}_{protein}-run{run}-{frame}.pdb".format(
                    run_index=run_index,
                    run_ind=run_ind,
                    protein=protein,
                    run=run,
            ...
            save_file = os.path.join(
                os.path.dirname(__file__),
                "cpet",
                "{run_index}_{run_ind}_{protein}-run{run}-{frame}.html".format(
                    run_index=run_index,
                    run_ind=run_ind,
                    protein=protein,
                    run=run,
            ...
            # run command to plot
            os.system(
                "plot_field_on_struct.py -options {} -pdb_file {} -field_file {}".format(
                    json_name, pdb_file, field_file
                )
            )
