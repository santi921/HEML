...
if __name__ == "__main__":
    ...
    if dense:
        ...
        if pca_tf:
            ...
            y_train_pred = model_xgb.predict(
                X_train.reshape(
                    X_train.shape[0],
                    X_train.shape[1]
                    * X_train.shape[2]
        ...
        model = tf.keras.Sequential(
            [
                tf.keras.layers.Conv3D(
                    ...
                    activation="sigmoid",
                    padding="same",
                    kernel_regularizer=L1(10e-4),
                    activity_regularizer=L1(10e-4),
                ...
                tf.keras.layers.Dense(
                    np.shape(y)[1], activation="softmax", name="visualized_layer"
                ),
            ]
        )
    ...
    if pca_tf:
        ...
        model.fit(
            X_train, y_train, epochs=10, validation_data=(X_test, y_test), verbose=True
        )
    ...
    x, y, z = np.meshgrid(
        np.arange(-3, 2.8, 0.2), np.arange(-3, 2.8, 0.2), np.arange(-3, 2.8, 0.2)
    )
    iron_size = 1.26
    resolution = 100
