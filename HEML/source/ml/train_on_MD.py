...
class training:
    def __init__(self, model, pca_tf=True, aug=True, test_crystal=False):
        self.aug = aug
        self.pca_tf = pca_tf
        self.model = model
    ...
    def train(self):
        ...
        with wandb.init(project="HemeML_redux") as run:
            ...
            if self.test_crystal:
                ...
                wandb.log({"crystal_acc": accuracy_score(self.y_crystal, model_obj.predict(self.x_crystal))})
                print("crystal f1 score: {:.2f}".format(f1_score(self.y_crystal, model_obj.predict(self.x_crystal), average="weighted")))
                wandb.log({"crystal_f1": f1_score(self.y_crystal, model_obj.predict(self.x_crystal), average="weighted")})
...
if __name__ == "__main__":
    ...
    training_obj = training(model=model, pca_tf=pca_tf, test_crystal=True)
