...
class training: 
    ...
    def train(self):
        ...
        for ind_train, ind_val in kf.split(X=self.X_train, y=self.y_train): 
            ...
            if(model!="ridge"):
                auroc_val.append(roc_auc_score(y_val_temp, self.model_obj.predict_proba(x_val_temp), 
                        multi_class="ovr"))
        ...
        if(model!="ridge"):
            roc_auc_test = roc_auc_score(self.y_test, self.model_obj.predict_proba(self.X_test), 
                        multi_class="ovr")            
            print("validation auroc: {:.2f}".format(np.mean(np.array(auroc_val))))
            print("test auroc:       {:.2f}".format(roc_auc_test))

    def boruta(self): 
        feat_selector = BorutaPy(self.model_obj, n_estimators=100, verbose=1, random_state=42, max_iter=2000)
    ...
    def perm_imp(self):
        self.make_model()
        self.train()
        r = permutation_importance(self.model_obj, self.X_train, self.y_train,
                           n_repeats=50,
                           random_state=0)
