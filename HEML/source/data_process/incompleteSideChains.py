...
for m in mols:
    for r in m.residues:
        ...
        # mutate to gly/ala
        if "CB" not in r_amap:
            replyobj.info("Mutating %s (incomplete " "side chain) to GLY\n" % str(r))
            r.type = "GLY"
            continue
        replyobj.info("Mutating %s (incomplete side" " chain) to ALA\n" % str(r))
        r.type = "ALA"
        seen = set()
...
if incompleteSideChains == "rotamers":
    if targets:
        replyobj.info("Residues with incomplete" " side chains:\n")
        for t in targets:
            replyobj.info("\t" + str(t) + "\n")
        replyobj.info("Replacing each by 'swapaa" " same (residue atom spec)")
