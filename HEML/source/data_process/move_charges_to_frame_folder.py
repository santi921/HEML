...
def move_charges_into_folder(charges_root, compressed_frame_folder):
    ...
    for i in folders:
        ...
        if os.path.exists(os.path.join(charges_root, frame + "_" + protein + suffix)):
            ...
            os.system(
                "cp "
                + os.path.join(charges_root, frame + "_" + protein + suffix)
                + " "
                + os.path.join(compressed_frame_folder, i + "/" + i + "_charges.pqr")
            )
        else:
            print(i + " not found")


move_charges_into_folder("../charges_2/processed_charges/", "./")
